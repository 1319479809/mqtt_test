...
func PersonFind(c *gin.Context) {
	...
	var res VgvfReplyData
	err = json.Unmarshal(body, &res)
	if err != nil {
		fmt.Println("SPersonFind err4=", err.Error())
		return
	}
	fmt.Println("SPersonFind res=", res)
	fmt.Println("SPersonFind res.Data=", res.Data)
	info := res.Data.(map[string]interface{})
	log.Println("info ====", info)
	pageInfo := info["pageInfo"]

	log.Println("pageInfo ====", pageInfo)
	// 打印返回结果
	//fmt.Println(string(body), resp.StatusCode)
	c.JSON(200, res)
